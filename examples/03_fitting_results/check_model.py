...
'''
Performs an excited-state structure optimization using the PEHS of the model.
If the minimum energy is unconstrained (tends towards -Infinity), try some more fitting!
There should be a well-defined minimum close to the expected value from ab-initio calculations
or experiment.
'''
...
from vchamtools.vcham import vcham, err_nstate
...
def fun_wrapper(p, *args):
    return err_nstate.Vn_adiab(p, *args)[0,1]

Q_guess = np.array([0.1, -0.1, -0.1, -0.1])

start = timer()
# CG, L-BFGS-B  #  SLSQP, BFGS
res = minimize(fun_wrapper, Q_guess, args=(H.w, H.E, H.c_kappa, H.c_gamma, H.c_rho, H.c_sigma, H.c_lambda, H.c_eta),
...
Ex = err_nstate.Vn_adiab(res.x, H.w, H.E, H.c_kappa, H.c_gamma, H.c_rho, H.c_sigma, H.c_lambda, H.c_eta)
print('DeltaE to GS:', Ex[0,1] - Ex[0,0], 'eV')
